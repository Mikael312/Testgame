...
local HIDE_SKIN_DELAY = 2.0 -- PENTING: Lebih lambat dari FPS Boost (0.5s)
...
local function destroySentry(desc)
    ...
    followConnection = RunService.RenderStepped:Connect(function()
        if not running or not sentryEnabled or not sentryExists(desc) then
            if followConnection then followConnection:Disconnect(); followConnections[desc] = nil end
            return
        end
    ...
    task.spawn(function()
        ...
        spamConnection = RunService.Heartbeat:Connect(function()
            if not sentryEnabled or not sentryExists(desc) then
                ...
                if not sentryExists(desc) then print("[✅] Enemy sentry DESTROYED! Total hits: " .. hitCount)
                else print("[⏹️] Attack stopped. Hits: " .. hitCount) end
                return
            end
        ...
        for slot, animalData in pairs(animalList) do
            if type(animalData) == "table" then
                ...
                table.insert(newCache, {
                    name = animalInfo.DisplayName or animalName, genValue = genValue, mutation = mutation,
                    traits = traits, owner = ownerName, plot = plot.Name, slot = tostring(slot),
...
local function enableXrayBase()
    ...
    cleanupConnection = workspace.DescendantRemoving:Connect(function(obj) if originalTransparency[obj] then originalTransparency[obj] = nil end end)
...
local function disableXrayBase()
    ...
    for obj, value in pairs(originalTransparency) do
        if obj and obj.Parent then
            pcall(function() obj.LocalTransparencyModifier = value; restoredCount = restoredCount + 1 end)
        end
    end
...
local function storeOriginalSettings()
    pcall(function()
        originalSettings = {
            streamingEnabled = workspace.StreamingEnabled, streamingMinRadius = workspace.StreamingMinRadius,
            streamingTargetRadius = workspace.StreamingTargetRadius, qualityLevel = settings().Rendering.QualityLevel,
            meshPartDetailLevel = settings().Rendering.MeshPartDetailLevel, globalShadows = game.Lighting.GlobalShadows,
            brightness = game.Lighting.Brightness, fogEnd = game.Lighting.FogEnd, technology = game.Lighting.Technology,
            environmentDiffuseScale = game.Lighting.EnvironmentDiffuseScale,
            environmentSpecularScale = game.Lighting.EnvironmentSpecularScale, decoration = workspace.Terrain.Decoration,
            waterWaveSize = workspace.Terrain.WaterWaveSize, waterWaveSpeed = workspace.Terrain.WaterWaveSpeed,
...
local PERFORMANCE_FFLAGS = {
    ...
    ["FFlagDebugGraphicsDisableDirect3D11"] = true, ["FFlagDebugGraphicsPreferD3D11FL10"] = false,
    ["DFFlagDebugRenderForceTechnologyVoxel"] = true, ["FFlagDisablePostFx"] = true,
    ["FIntRenderShadowIntensity"] = 0, ["FIntRenderLocalLightUpdatesMax"] = 0,
    ["FIntRenderLocalLightUpdatesMin"] = 0, ["DFIntTextureCompositorActiveJobs"] = 1,
    ["DFIntDebugFRMQualityLevelOverride"] = 1, ["FFlagFixPlayerCollisionWhenSwimming"] = false,
    ["DFIntMaxInterpolationSubsteps"] = 0, ["DFIntS2PhysicsSenderRate"] = 15,
    ["DFIntConnectionMTUSize"] = 1492, ["DFIntHttpCurlConnectionCacheSize"] = 134217728,
    ["DFIntCSGLevelOfDetailSwitchingDistance"] = 0, ["DFIntCSGLevelOfDetailSwitchingDistanceL12"] = 0,
    ["DFIntCSGLevelOfDetailSwitchingDistanceL23"] = 0, ["DFIntCSGLevelOfDetailSwitchingDistanceL34"] = 0,
    ["FFlagEnableInGameMenuChromeABTest3"] = false, ["FFlagEnableInGameMenuModernization"] = false,
    ["FFlagEnableReportAbuseMenuRoactABTest2"] = false, ["FFlagDisableNewIGMinDUA"] = true,
    ["FFlagEnableV3MenuABTest3"] = false, ["FIntRobloxGuiBlurIntensity"] = 0,
    ["DFIntTimestepArbiterThresholdCFLThou"] = 10, ["DFIntTextureQualityOverride"] = 1,
    ["DFIntPerformanceControlTextureQualityBestUtility"] = 1, ["DFIntTexturePoolSizeMB"] = 64,
    ["DFIntMaxFrameBufferSize"] = 1, ["FFlagDebugDisableParticleRendering"] = false,
    ["DFIntParticleMaxCount"] = 100, ["FFlagEnableWaterReflections"] = false,
    ...
    pcall(function()
        for _, obj in ipairs(workspace:GetDescendants()) do
            pcall(function()
                ...
                elseif obj:IsA("BasePart") then obj.CastShadow = false; obj.Material = Enum.Material.Plastic; if obj.Material == Enum.Material.Glass then obj.Reflectance = 0 end end
            end)
        end
    end)
end
        ...
        pcall(function()
            for _, part in ipairs(char:GetDescendants()) do
                pcall(function()
                    ...
                    elseif part:IsA("Fire") or part:IsA("Smoke") or part:IsA("Sparkles") then part:Destroy() end
                end)
            end
        end)
    end)
...
local function disableFpsBoost()
    ...
    pcall(function()
        ...
        game.Lighting.GlobalShadows = originalSettings.globalShadows ~= false; game.Lighting.Brightness = originalSettings.brightness or 1; game.Lighting.FogEnd = originalSettings.fogEnd or 100000; game.Lighting.Technology = originalSettings.technology or Enum.Technology.ShadowMap
        game.Lighting.EnvironmentDiffuseScale = originalSettings.environmentDiffuseScale or 1; game.Lighting.EnvironmentSpecularScale = originalSettings.environmentSpecularScale or 1
        workspace.Terrain.WaterWaveSize = originalSettings.waterWaveSize or 0.15; workspace.Terrain.WaterWaveSpeed = originalSettings.waterWaveSpeed or 10; workspace.Terrain.WaterReflectance = originalSettings.waterReflectance or 1; workspace.Terrain.WaterTransparency = originalSettings.waterTransparency or 0.3; workspace.Terrain.Decoration = originalSettings.decoration ~= false
    end)
...
local function disableTimerESP()
    ...
    for _, plot in pairs(Workspace:FindFirstChild("Plots"):GetChildren()) do
        ...
        if purchases then
            for _, purchase in pairs(purchases:GetChildren()) do
                local main = purchase:FindFirstChild("Main")
                if main then local gui = main:FindFirstChild("RemainingTimeGui"); if gui then gui:Destroy() end end
            end
        end
    end
...
local function cleanCharacter(character)
    ...
    if processedCharacters[character] then return end
    
    local waitTime = HIDE_SKIN_DELAY
    if fpsBoostEnabled then
        waitTime = 2.5
        print("⏳ Waiting for FPS Boost to complete...")
    end
    ...
    if success then
        ...
        print("🧹 Hide Skin cleaned: " .. (character.Name or "Unknown"))
    end
end
    ...
    local conn = character.ChildAdded:Connect(function(child)
        ...
        if child:IsA("Accessory") or child:IsA("Hat") or 
           ...
           child:IsA("ShirtGraphic") or child:IsA("CharacterMesh") then
            child:Destroy()
            print("🚫 Hide Skin blocked: " .. child.Name)
        end
    end)
    
    table.insert(hideSkinConnections, conn)
end

local function enableHideSkin()
    if hideSkinRunning then warn("⚠️ Hide Skin already running!"); return false end
    
    hideSkinRunning = true
    processedCharacters = {}
    
    print("🔄 Hide Skin starting...")
    if fpsBoostEnabled then print("⚠️ FPS Boost detected - using delayed mode") end
    ...
    local conn1 = Players.PlayerAdded:Connect(function(plr)
        ...
        table.insert(hideSkinConnections, conn2)
    end)
    table.insert(hideSkinConnections, conn1)
    ...
    local cleanupConn = RunService.Heartbeat:Connect(function()
        if not hideSkinRunning then return end
        for char, _ in pairs(processedCharacters) do
            if not char or not char.Parent then processedCharacters[char] = nil end
        end
    end)
    table.insert(hideSkinConnections, cleanupConn)
    
    print("✅ Hide Skin Enabled (No-Conflict Mode)")
    return true
end

local function disableHideSkin()
    if not hideSkinRunning then warn("⚠️ Hide Skin not running!"); return false end
...
local function toggleAutoSteal(state) if state then enableAutoSteal() else disableAutoSteal() end end
...
local function toggleHideSkin(state)
    if state then enableHideSkin() else disableHideSkin() end
end
...
Players.PlayerAdded:Connect(function(targetPlayer)
    targetPlayer.CharacterAdded:Connect(function(character)
        task.wait(1)
        if espPlayersEnabled and targetPlayer ~= player then createESP(targetPlayer) end
    end)
end)
...
for _, targetPlayer in pairs(Players:GetPlayers()) do
    if targetPlayer ~= player then
        targetPlayer.CharacterAdded:Connect(function(character)
            task.wait(1)
            if espPlayersEnabled then createESP(targetPlayer) end
        end)
    end
end

-- UPDATED player.CharacterAdded SECTION
player.CharacterAdded:Connect(function(newCharacter)
    task.wait(1)
    
    -- FPS Boost runs FIRST
    if fpsBoostEnabled then
        for _, p in ipairs(Players:GetPlayers()) do
            if p.Character then optimizeCharacter(p.Character) end
        end
    ...
    if espBestEnabled then
        updateHighestValueESP()
        if highestValueData then createTracerLine() end
    end
    ...
    -- Anti-Ragdoll V2 handles its own respawn logic internally
    ...
    if hideSkinRunning then
        task.spawn(function()
            ...
            print("🔄 Hide Skin reloaded after respawn")
        end)
    end
end)
