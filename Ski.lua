--[[
    ARCADE UI - INTEGRASI ESP PLAYERS, ESP BEST, BASE LINE, ANTI TURRET, AIMBOT, KICK STEAL, UNWALK ANIM, AUTO STEAL, ANTI DEBUFF, ANTI KB, XRAY BASE & FPS BOOST (FIXED VERSION)
]]
...
local baseBeam = nil
local baseLineStable = false -- New variable to track line stability
...
local function enableESPPlayers()
    ...
    -- FIXED: Use Heartbeat instead of RenderStepped for more consistent updates
...
local function checkBaseLineIntegrity()
    if not baseLineEnabled then return end
    
    -- Check if any component is missing
    local componentsMissing = not baseBeamPart or not baseTargetPart or not baseBeam
    
    if componentsMissing or not baseLineStable then
        print("🔧 Base Line components missing or unstable, recreating...")
        pcall(stopPlotLine)
        task.wait(0.5)
...
local function enableBaseLine()
    ...
    -- FIXED: Add periodic check for line integrity
    task.spawn(function()
        while baseLineEnabled do
            task.wait(2)
...
player.CharacterAdded:Connect(function(newCharacter)
    ...
    if espBestEnabled then
        updateHighestValueESP()
        -- FIXED: Explicitly recreate tracer after respawn
        task.wait(0.5)
